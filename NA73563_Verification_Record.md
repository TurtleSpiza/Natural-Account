...
- L4 Tax GREEN on the sighted lines, AMBER on the unsighted population. WINC ABN 94 000 728 398 checksum PASS, GST-registered. Officeworks ABN 36 004 763 526 checksum PASS; the TE005250 receipt is under the $1,000 tier. Invoice-total GST reconciles on the sighted invoices.
- **GST status (Limb 4, per-line).** Invoice-total GST reconciles, but the per-line food check is not complete. The recoded refreshment lines (coffee, Milo, hot chocolate, sugar, ice blocks, confectionery) include GST-free basic-food items that must not carry an input tax credit; each must be reconciled against the WINC line-level taxable flag, not the invoice total. This cannot finish until the nine unsighted WINC invoices are sighted, so the refreshment population is AMBER pending sighting, GREEN on the sighted stationery and IT lines. Tax does not clear ahead of evidence.
